"""
[Code](https://github.com/jamjamjon/usls/blob/main/scripts/sam3-image/export_v2.py) adapted from jamjamjon's usls cross platform rust library
...
def export_vision_encoder(
    ...
    print(f"Exporting vision encoder...")
    model_name = "sam3_image_encoder.onnx"
    ...
    print(f"Vision encoder export complete.  Model saved to {output_dir / 'sam3_image_encoder.onnx'}")
    if use_external_data:
        print("Creating external data for vision encoder...")
        import onnx
        onnx_model = onnx.load_model(str(model_path))
        onnx.save_model(onnx_model, str(model_path), save_as_external_data=True, all_tensors_to_one_file=True, location=f"{model_name}.data", size_threshold=0, convert_attribute=False)
        print(f"External data for vision encoder saved to {output_dir / (model_name + '.data')}")
...
def export_text_encoder(
    ...
    model_name = "sam3_text_encoder.onnx"
    model_path = output_dir / model_name
    print(f"Exporting text encoder...")
    torch.onnx.export(
        wrapper,
        (
    ...
    print(f"Text encoder export complete.  Model saved to {output_dir / 'sam3_text_encoder.onnx'}")
    if use_external_data:
        print("Creating external data for text encoder...")
        import onnx
        onnx_model = onnx.load_model(str(model_path))
        onnx.save_model(onnx_model, str(model_path), save_as_external_data=True, all_tensors_to_one_file=True, location=f"{model_name}.data", size_threshold=0, convert_attribute=False)
        print(f"External data for text encoder saved to {output_dir / (model_name + '.data')}")
...
def export_decoder(
    ...
    print(f"Decoder export complete.  Model saved to {output_dir / 'sam3_decoder.onnx'}")
    if use_external_data:
        print("Creating external data for decoder...")
        import onnx
        onnx_model = onnx.load_model(str(model_path))
        onnx.save_model(onnx_model, str(model_path), save_as_external_data=True, all_tensors_to_one_file=True, location=f"{model_name}.data", size_threshold=0, convert_attribute=False)
        print(f"External data for decoder saved to {output_dir / (model_name + '.data')}")
...
def main():
    ...
    parser.add_argument("--use-external-data", default=False, action="store_true", help="Store weights for models in onnx.data files for faster loading.")
    ...
    model = Sam3Model.from_pretrained("facebook/sam3").to(args.device).eval()
