...
class Sam3ONNXInference:
    def __init__(
        ...
        self.tokenizer = Tokenizer(bpe_path=bpe_path, context_length=32)

    def _preload_ort_dlls(self):
        cuda_path, cudnn_path = get_nvidia_lib_paths()
        ort.preload_dlls(cuda = True, directory=cuda_path)
        ort.preload_dlls(cudnn=True, directory=cudnn_path)
    ...
    def encode_text(self, text: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        attention_mask = np.array(input_ids != 0, dtype=np.int64)
